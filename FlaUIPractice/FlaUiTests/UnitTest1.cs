...
namespace FlaUiTests
{
    ...
    public class UnitTest1
    {
        ...
        [TestMethod]
        public void TestUIAProperties()
        {
            var application = FlaUI.Core.Application.Launch(@"C:\Data\Visual Studio Workspace\FlaUiPractice\FlaUIPractice\FlaUiTests\Resources\WpfApplication.exe");
            var automation = new UIA3Automation();
            var mainWindow = application.GetMainWindow(automation);

            var checkbox = mainWindow.FindFirstDescendant(cf => cf.ByAutomationId("SimpleCheckBox"));
            Console.WriteLine(checkbox.Properties.Name);
            Console.WriteLine(checkbox.Properties.ControlType);
            mainWindow.Close();
        }
        ...
        public void TestCalculator()
        {
            ...
            using (var automation = new UIA3Automation())
            {
                ...
                mainWindow.FindFirstDescendant(cf.ByName("One")).AsButton().Click();
                mainWindow.FindFirstDescendant(cf.ByName("Plus")).AsButton().Click();
                mainWindow.FindFirstDescendant(cf.ByName("One")).AsButton().Click();
                mainWindow.FindFirstDescendant(cf.ByName("Equals")).AsButton().Click();
                var displayBox = mainWindow.FindFirstDescendant(cf.ByAutomationId("CalculatorResults").And(cf.ByControlType(ControlType.Text)));
                var displayName = displayBox.Properties.Name;
                Assert.AreEqual("Display is 2", displayName);
            }
        }

        [TestMethod]
        public void TestWordPad()
        {
            var app = Application.Launch(@"C:\Program Files\Windows NT\Accessories\wordpad.exe");
